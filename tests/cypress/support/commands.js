...
Cypress.Commands.add("login", (username, password) => {
  cy.getCookies().then((cookies) => {
    ...
    if (!hasMatch) {
      ...
      cy.visit(
        "http://localhost:10008/wp-admin/admin.php?page=bluehost#/home/store/general"
      );
    }
  });
});
...
Cypress.Commands.add(
  "sso_login",
  (domain_username, domain_password, bluehost_username, bluehost_password) => {
    cy.getCookies().then((cookies) => {
      let hasMatch = false;
      cookies.forEach((cookie) => {
      ...
      if (!hasMatch) {
        ...
        cy.get("[data-testid=login-wordpress]", { timeout: 30000 }).as(
          "login-wordpress"
        );
