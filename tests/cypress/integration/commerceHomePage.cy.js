...
describe(
	...
	() => {
		...
		it('Verify Congrats on your new site message, coming soon alert', () => {
			...
			cy.get('.nfd-alert.nfd-alert--warning').as('comingsoonalert').should('exist')
		...
		it('Verify presense of Ready to go to live? canvas', () => {
			cy.get('.nfd-px-4').as('readyToGoNextLevel').should('exist', { timeout: customCommandTimeout })

			cy.get('@readyToGoNextLevel').find('.nfd-flex-1 h1').should('have.text', 'Ready to go live?')
			...
			cy.get('@readyToGoNextLevel').find('.nfd-flex-none > .nfd-button--secondary').should('exist').and('have.text', 'View your site')
		...
		it('Verify Visit your site and Launch your site functionality', () => {
			...
			cy.get('.nfd-flex-none > .nfd-button--secondary').invoke('removeAttr', 'target').click()
