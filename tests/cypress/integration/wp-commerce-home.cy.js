...
describe('As a wp-admin user, I want to ', function () {
    ...
    cy.findByTitle('Launch Your Site')
      ...
      .then(($element) => {
        ...
        tabList.forEach((element) => {
          cy.findByLabelText('Setup Guide').find('a>li').contains(element);
        });
      });
  });
  ...
  it('select, "I dont charge sales taxes"', () => {
    ...
    cy.get('[data-variant=minimal]', { timeout: customCommandTimeout }).should(
      (completedCards) => {
  ...
  it("verify 'Add a Product', 'Manage a Product', 'Categories', 'Tags' redirecting to right page after click", () => {
    ...
    cy.get('button[data-variant=minimal]', { timeout: customCommandTimeout })
      .as('Cards')
      .eq(0)
      .click();
  ...
  it('create home page, about page, contcat page', () => {
    ...
    cy.contains('Contact Page', { timeout: customCommandTimeout }).click();
