...
export function verifyStoreInfoCardIsInDoneState(){
    GeneralSettingPage.completedCards().eq(0).find('div').should('have.length',1)
...
export function verifyEnteredStoreAddressIsSameOnWooCommerceSetting(){
    ...
    WooCommercePage.addressLine1().should('have.value',data.store_address.address1)
...
export function linkExistingShippoAccount(){
    ...
    GeneralSettingPage.shippingAPI().clear().type(data.existing_shipping.test_api)
    GeneralSettingPage.senderName().clear().type(data.existing_shipping.sender_name)
    GeneralSettingPage.companyName().clear().type(data.existing_shipping.sender_company)
...
export function verifyFilledShippingInformationSavedInYith(){
    ...
    YithShippoPage.selectedShippoEnv().should('include.value',data.existing_shipping.environment)
    YithShippoPage.shippoAPI().should('have.value',data.existing_shipping.test_api)
    
    YithShippoPage.shippoShippingTab().eq(1).click()
    YithShippoPage.senderName().should('have.value',data.existing_shipping.sender_name)
    YithShippoPage.compnayName().should('have.value',data.existing_shipping.sender_company)
...
export function setStandardRate(){
    ...
    WooCommercePage.countryRowList().eq(0).find("input").type(data.standard_tax_setting.country_code)
    WooCommercePage.stateCodeRowList().eq(0).find('input').type(data.standard_tax_setting.state_code)
    WooCommercePage.taxRateRowList().eq(0).find('input').type(data.standard_tax_setting.state_code)
