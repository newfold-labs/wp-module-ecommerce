...
/**
 * Store Quick Start Class
 */
class StoreQuickStart {
    ...
    public function enqueue_scripts() {
        ...
        $asset_file = NFD_ECOMMERCE_BUILD_DIR.'store-quick-start/index.asset.php';
        if (file_exists($asset_file)) {
            $asset = require $asset_file;

            $asset['dependencies'][] = 'quick-add-product';

            // Register script.
            wp_register_script(
                'store-quick-start',
                NFD_ECOMMERCE_PLUGIN_URL.'vendor/newfold-labs/wp-module-ecommerce/build/store-quick-start/index.js',
                $asset['dependencies'],
                $asset['version'],
                true
            );
            ...
            wp_register_style(
                'store-quick-start-style',
                NFD_ECOMMERCE_PLUGIN_URL.'vendor/newfold-labs/wp-module-ecommerce/build/store-quick-start/store-quick-start.css',
                array( 'quick-add-product' ),
                $asset['version']
            );
            ...
            if ( isset( $_GET['page'] ) && 'bluehost' === sanitize_text_field( wp_unslash( $_GET['page'] ) ) ) {
                ...
                wp_enqueue_script('store-quick-start');
                wp_enqueue_style('store-quick-start-style');
            }
        }
    }
    ...
    public function output_root() {
        echo '<div id="nfd-store-quick-start"></div>';
    }
    ...
    protected function get_store_industries() {
        return $this->format_localized_array(array(
