...
export function useInstallWonderCart({ wpModules }) {
  ...
  let installWonderCart = useSWRMutation("install-nfd_slug_wonder_cart", async () => {
    ...
    if (response !== "failed") {
      ...
      await sleep(1000);
      window.location.href = window.NewfoldRuntime?.linkTracker?.addUtmParams("admin.php?page=bluehost#/store/sales_discounts" ) || "admin.php?page=bluehost#/store/sales_discounts";
